...
@implementation WXSTopicViewModel


+ (instancetype)viewModelWithTopic:(WXSTopic *)topic
{
    WXSTopicViewModel *viewModel = [[self alloc] init];
...
- (void)setTopic:(WXSTopic *)topic
{
    ...
    // 2计算图片的高度, 不是段子就用图片
    if (topic.type != WXSTopicTypeWords) {
...
- (YYTextLayout *)topCmtLayout
{
    if(_topCmtLayout == nil)
    {
        ...
        // 遍历添加评论
        [self.topic.topCmts enumerateObjectsUsingBlock:^(WXSTopicTopComment * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
